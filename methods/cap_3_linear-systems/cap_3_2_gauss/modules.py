...
def calc_multiplicators(matrix, pivot, iteration: int = 0):
    ...
    # print(f'matrix with multiplicator: \n {matrix_with_multiplicators} \n')

    # new_column_1 = matrix[:,0].copy().reshape(matrix.shape[0], 1)
    new_column_1 = matrix[:,iteration].copy().reshape(matrix.shape[0], 1)
    print('new_column_1: ', new_column_1, '\n')

    for i, coef in enumerate(matrix[1 + iteration:,0]):        
